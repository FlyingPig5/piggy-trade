...
public class MainActivity extends AppCompatActivity {
    ...
    public void onBackPressed() {
        ...
        try {
            // Bypass IPythonApp proxy — access the live Python app instance directly
            Python py = Python.getInstance();
            PyObject appModule = py.getModule("piggytrade.app");
            PyObject appInstance = appModule.get("_app_instance");
            if (appInstance != null) {
                PyObject result = appInstance.callAttr("onBackPressed");
                if (result != null) {
                    allowDefault = result.toBoolean();
                }
            } else {
                Log.w(TAG, "onBackPressed: _app_instance is null");
            }
        } catch (Exception e) {
            Log.e(TAG, "onBackPressed: Python call failed: " + e.getMessage());
        }
        if (allowDefault) {
            super.onBackPressed();
        }
